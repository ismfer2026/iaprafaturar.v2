...
import { sendPublicReferralChatMessage } from "@/lib/public-referral-api";
...
export default function PublicIndicacaoPage() {
  ...
  async function invokeChat(input: {
    message: string;
    conversation: Array<{ role: string; content: string }>;
    collectedData: Record<string, unknown>;
  }) {
    const response = await sendPublicReferralChatMessage({
      code: codigo,
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      setIsThinking(true);
      try {
        const data = await invokeChat({ message: "INICIO", conversation: [], collectedData: {} });
        const reply = data.reply || t("indicacao.description");
  ...
  async function handleSendMessage(event: FormEvent) {
    ...
    try {
      const data = await invokeChat({ message: text, conversation: nextConversation, collectedData });
      ...
      console.error("[PublicIndicacaoPage] public-referral-handler error:", error);
      appendMessage("agente", t("common.error"));
    } finally {
      setIsThinking(false);
    }
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-teal-50 to-white p-4">
        <div className="max-w-md space-y-4 text-center">
          ...
          <p className="text-sm text-zinc-500">{t("indicacao.notFound")}</p>
          {errorDetail ? (
      ...
      >
        <div className="w-full max-w-md space-y-6 text-center text-white">
          ...
          <div className="space-y-2">
            <h1 className="text-3xl font-black">{t("indicacao.doneTitle")}</h1>
            <p className="text-sm leading-6 text-white/70">{t("indicacao.done")}</p>
          </div>
    ...
    <div style={{ minHeight: "100dvh", background: "#F8FFFE", display: "flex", flexDirection: "column" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ color: "#fff", fontWeight: 700, fontSize: 14 }}>{t("indicacao.chatTitle")}</div>
            <div style={{ color: "rgba(255,255,255,0.75)", fontSize: 11 }}>{t("indicacao.chatSubtitle")}</div>
          </div>
        </div>
      </div>
                ...
                {[0, 150, 300].map((delay) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "50%",
                      backgroundColor: "#0D6E6E",
                      animation: "indicacao-bounce 1s infinite",
      ...
      >
        <form onSubmit={handleSendMessage} style={{ display: "flex", gap: 8 }}>
          <Input
            ref={inputRef}
            value={inputValue}
            placeholder={t("indicacao.inputPlaceholder")}
            disabled={isThinking}
