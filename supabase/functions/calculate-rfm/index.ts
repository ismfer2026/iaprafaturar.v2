import {
  validateCalculateRfmInput,
  validateCalculateRfmOutput,
} from '@iaprafaturar/contracts/edge-functions/calculate-rfm.ts'
...
const AGENT_SLUG = 'calculate-rfm'
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const input = validateCalculateRfmInput(await request.json())
    ...
    for (const professional of professionals) {
      const { data, error } = await supabase.rpc('calculate_rfm_for_professional', {
    ...
    return jsonResponse(validateCalculateRfmOutput({
