-- ============================================================
-- Phase 10 - Professional Reports & Intelligence
-- Scope:
-- - No new tables.
-- - Read-only RPC for professional reports.
-- - Data scoped exclusively by auth_professional_id().
-- ============================================================

DROP FUNCTION IF EXISTS public.get_professional_reports_rpc(date);
...
BEGIN
  ...
  v_start_ts := v_month_start::timestamptz;
  v_end_ts := v_month_end::timestamptz;
  ...
  SELECT jsonb_build_object(
    ...
    'revenuePending', (
      ...
      WHERE ft.professional_id = v_professional_id
        ...
        AND COALESCE(ft.due_date, ft.created_at::date) >= v_month_start
        AND COALESCE(ft.due_date, ft.created_at::date) < v_month_end
    ),
    'expensesPaid', (
      SELECT COALESCE(SUM(ft.net_amount), 0)
      FROM public.financial_transactions ft
...
COMMENT ON FUNCTION public.get_professional_reports_rpc(date) IS
  'Read-only professional reports RPC. Uses auth_professional_id() and does not accept professional_id payload.';
